    ...
    return BlocConsumer<AppCubit,AppStates>(listener: (BuildContext context, state) {
      ...
      builder: (BuildContext context, state) {
        return  ConditionalBuilder(
          condition: AppCubit.get(context).technology.length!=0,
          builder: (BuildContext context) {
            return ListView.separated(itemBuilder: (context,index)=>defaultListItem(AppCubit.get(context).technology[index],context), separatorBuilder: (context,index)=>Container(), itemCount: AppCubit.get(context).sports.length);
          },
